...
import { DocumentChainVisualizer } from './DocumentChainVisualizer';
...
export function Dashboard({ onLogout, isDemo = false }: DashboardProps) {
  ...
  const filteredAccounts = (() => {
    ...
    filteredEntities.forEach(entity => {
      entity.accounts?.forEach(accountId => {
    ...
    if (allowedAccountIds.size === 0) {
      return accountsList; // Show all if no specific accounts linked
    }
    
    return accountsList.filter(account => allowedAccountIds.has(account.id));
  })();

  // Calculate total balance in KZT using filtered accounts
  const totalBalanceKZT = filteredAccounts.reduce((sum, acc) => {
  ...
  const filteredTransactions = transactionsList
    .filter(t => {
      // Organization filter - only show transactions from filtered accounts
      const account = filteredAccounts.find(a => a.name === t.account);
      if (!account) return false;
      
      // Existing filters
      if (selectedCountry && t.country !== selectedCountry) return false;
      const accountCurrency = accounts.find(a => a.name === t.account)?.currency;
    ...
    .sort((a, b) => {
      ...
      switch (transactionSort) {
        ...
        case 'amount':
          ...
          break;
        case 'counterparty':
          comparison = (a.counterparty?.name || '').localeCompare(b.counterparty?.name || '');
          break;
      }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl p-6 text-white shadow-xl hover:shadow-2xl transition-all">
                  ...
                  <p className="text-3xl mb-1">
                    {formatDisplayCurrency(totalBalanceKZT / currencyRates[displayCurrency])}
                  </p>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-lg transition-all">
                  ...
                  <p className="text-2xl text-gray-900 mb-1">
                    {formatDisplayCurrency(convertToDisplayCurrency(10700, 'USD'))}
                  </p>
                  <p className="text-sm text-green-600 flex items-center gap-1">
                    <TrendingUp className="w-3 h-3" />
                    +18.2% {language === 'ru' ? 'за месяц' : 'vs last month'}
                  </p>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-lg transition-all">
                  ...
                  <p className="text-2xl text-gray-900 mb-1">
                    {formatDisplayCurrency(convertToDisplayCurrency(8450, 'USD'))}
                  </p>
                  <p className="text-sm text-orange-600 flex items-center gap-1">
                    <TrendingUp className="w-3 h-3" />
                    +5.1% {language === 'ru' ? 'за месяц' : 'vs last month'}
                  </p>
                </div>
                    ...
                    <div
                      ...
                      onClick={() => {
                        ...
                        if (selectedCurrency === account.currency && activeSection === 'transactions') {
                          ...
                          toast.info(language === 'ru' 
                            ? `Показ операций для ${account.name}` 
                            : `Showing transactions for ${account.name}`
                          );
                        }
                      }}
                      className={`bg-gradient-to-br ${account.color} rounded-xl p-5 text-white shadow-lg hover:shadow-xl hover:scale-105 transition-all cursor-pointer group relative ${
                        selectedCurrency === account.currency && activeSection === 'transactions' ? 'ring-4 ring-white ring-offset-2' : ''
                      }`}
                    >
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-2xl group-hover:scale-110 transition-transform">{account.flag}</span>
                        <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm text-white/80 mb-2 group-hover:text-white transition-colors">{account.name}</p>
                      <p className="text-2xl mb-1 group-hover:scale-105 transition-transform">{formatCurrency(account.balance, account.currency)}</p>
                      <p className="text-xs text-white/70 group-hover:text-white/90 transition-colors flex items-center gap-2">
                        {account.currency}
                        {selectedCurrency === account.currency && activeSection === 'transactions' ? (
                          <span className="text-white/90">• {language === 'ru' ? 'Клик для сброса' : 'Click to reset'}</span>
                        ) : (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all group-hover:scale-110 $${
                          transaction.amount > 0 ? 'bg-green-100 group-hover:bg-green-200' : 'bg-red-100 group-hover:bg-red-200'
                        }`}>
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <p className="text-xs text-gray-500 group-hover:text-blue-600 transition-colors">{transaction.account}</p>
                            <span className="text-xs text-gray-400">•</span>
                            <p className="text-xs text-gray-500 group-hover:text-blue-600 transition-colors">{transaction.category}</p>
                            <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-md group-hover:bg-blue-600 group-hover:text-white transition-colors">
                      ...
                      <div className="text-right">
                        <p className={`text-sm ${transaction.amount > 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {transaction.amount > 0 ? '+' : ''}
                          {formatCurrency(transaction.amount, accounts.find(a => a.name === transaction.account)?.currency || 'USD')}
                        </p>
                  ...
                  <div className="bg-white rounded-xl border-2 border-gray-200 p-6 shadow-sm">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                      <h3 className="text-lg text-gray-900 flex items-center gap-2">
                        ...
                        {language === 'ru' ? 'Настройки синхрониза��ии' : 'Sync Settings'}
                      </h3>
                      <div className="relative">
                        <select
                          value={syncFrequency}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="group bg-white rounded-2xl p-6 border border-gray-200 shadow-sm hover:shadow-md transition-all">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-gray-500 text-sm">{language === 'ru' ? 'Доход' : 'Income'}</span>
                  </div>
                  <h3 className="text-3xl text-gray-900 mb-1">
                    {filteredTransactions.filter(t => t.type === 'incoming').length}
                  </h3>
                  <p className="text-gray-600 text-sm">{language === 'ru' ? 'Поступлений' : 'Incoming'}</p>
                </div>
                ...
                <div className="group bg-white rounded-2xl p-6 border border-gray-200 shadow-sm hover:shadow-md transition-all">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-gray-500 text-sm">{language === 'ru' ? 'Расход' : 'Expense'}</span>
                  </div>
                  <h3 className="text-3xl text-gray-900 mb-1">
                    {filteredTransactions.filter(t => t.type === 'outgoing').length}
                  </h3>
                  <p className="text-gray-600 text-sm">{language === 'ru' ? 'Списаний' : 'Outgoing'}</p>
                </div>
                        ...
                        <tr className="bg-gray-50 border-b border-gray-200">
                          ...
                          <th className="px-4 py-3 text-left text-xs text-gray-600 uppercase tracking-wider">
                            <div className="flex items-center gap-2">
                              ...
                              <span>{language === 'ru' ? 'Цепочка' : 'Chain'}</span>
                            </div>
                          </th>
