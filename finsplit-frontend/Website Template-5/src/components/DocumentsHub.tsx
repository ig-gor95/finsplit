...
import { FileText, Upload, Plus, Eye, Download, Mail, BookOpen } from 'lucide-react';
import { DocumentsSection } from './DocumentsSection';
import { DocumentRequirementsTable } from './DocumentRequirementsTable';
...
type TabType = 'generate' | 'uploaded' | 'requirements';
...
export function DocumentsHub({
  ...
  const t = (key: string) => {
    const translations: Record<string, Record<string, string>> = {
      ...
      'tabs.requirements': { en: 'Requirements', ru: 'Требования' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border-2 border-gray-200 p-6 shadow-sm">
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            <div className="text-left">
              <div className="font-medium">{t('tabs.requirements')}</div>
              <div className={`text-xs ${activeTab === 'requirements' ? 'text-white/80' : 'text-gray-500'}`}>
                {language === 'ru' ? 'Справочник' : 'Reference'}
              </div>
            </div>
          </button>
        </div>
      ...
      {activeTab === 'requirements' && (
        <DocumentRequirementsTable language={language} />
      )}
    </div>
  );
}